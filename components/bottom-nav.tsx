...
export default function BottomNav({ activeTab, notificationCount = 0 }: BottomNavProps) {
  return (
    <nav
      className="w-full min-w-0 h-full min-h-[56px] bg-card rounded-t-2xl shadow-lg border border-border flex items-center justify-around pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]"
      role="navigation"
      aria-label="메인 네비게이션"
    >
        {TAB_CONFIG.map((tab) => {
          ...
          return (
            <Link
              key={tab.id}
              href={tab.href}
              className={`relative flex flex-col items-center gap-0.5 px-4 pt-1 pb-2 rounded-xl transition-colors ${
