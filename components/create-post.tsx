    ...
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto overscroll-contain">
      <div className="flex flex-col min-h-full">
        ...
        <div>
          ...
          >
            {/* Me */}
            <div className="shrink-0 snap-start flex flex-col items-center gap-1.5 p-2.5 rounded-lg bg-primary/10 ring-2 ring-primary w-[72px] min-w-[72px]">
              <UserAvatar user={currentUser} size="md" />
              <span className="text-[11px] font-medium">{"나"}</span>
            </div>
            {/* Selected friends */}
            {selectedFriends.map((f) => (
              <div key={f.id} className="shrink-0 snap-start relative flex flex-col items-center gap-1.5 p-2.5 rounded-lg bg-muted w-[72px] min-w-[72px]">
                <button
            ...
            {Array.from({ length: Math.max(0, totalSlots - 1 - selectedFriends.length) }).map((_, i) => (
              <button
                ...
                className="shrink-0 snap-start flex flex-col items-center gap-1.5 p-2.5 rounded-lg border-2 border-dashed border-border w-[72px] min-w-[72px]"
              >
